...
def calculate_metrics(real_clip_distance_map,fake_clip_distance_map,label,iteration,evaluate_path):
    ...
    # 检查是否有数据
    if not real_clip_distance_list or not fake_clip_distance_list:
    ...
    def safe_log(x):
        if x <= 0:
            return -10  # 一个足够小的负数作为对数的下限
        return math.log(x)
    
    # standardization with safe log
    real_clip_distance_list = [safe_log(i) for i in real_clip_distance_list]
    fake_clip_distance_list = [safe_log(i) for i in fake_clip_distance_list]
    real_video_distance_list = [safe_log(i) for i in real_video_distance_list]
    fake_video_distance_list = [safe_log(i) for i in fake_video_distance_list]
